...
import { Timer, Loader2, ChevronRightIcon, BarChart } from "lucide-react";
  ...
  const handleNext = useCallback(() => {
    ...
    checkAnswer(undefined, {
      onSuccess: ({ percentageSimilar }) => {
        toast({
          title: `Your answer is ${percentageSimilar}% similar to the correct answeer.`,
  ...
  if (hasEnded) {
    return (
      <div className="absolute flex flex-col justify-center top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
        ...
        >
          View Statistics
          <BarChart className="w-4 ml-2 h-4" />
        </Link>
      </div>
    );
  }
